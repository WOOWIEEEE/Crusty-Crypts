...
namespace Dungeon
{
    partial class Window
    {
        ...
        static void GenerateMap()
        {
            ...
            Vector2[] d = new Vector2[] {new Vector2(0, -1), new Vector2(1, 0), new Vector2(0, 1), new Vector2(-1, 0),
            ...
            for (int i = 0; i < w.MaxRoom; i++)
            {
                ...
                int[] OrderX = RandomiseArray((int)w.MapSize.X);
                int[] OrderY = RandomiseArray((int)w.MapSize.Y);
                for (int xi = 0; xi < w.MapSize.X; xi++)
                {
                    for (int yi = 0; yi < w.MapSize.Y; yi++)
                    {
                        ...
                        Vector2 n = new Vector2(x, y) + d[r.Direction];
                        if (!OnMap(n)) continue;
                        if (w.Map[(int)n.Y, (int)n.X] == 2)
                        {
                            ...
                            for (int a = 0; a < r.Floor.Count(); a++)
                            {
                                ...
                                if (!OnMap(n) || w.Map[(int)n.Y, (int)n.X] != 0)
                                {
                                    Fail = true;
                                    goto Finish1;
                                }
                            ...
                            for (int a = 0; a < r.Wall.Count(); a++)
                            {
                                ...
                                if (!OnMap(n) || !(w.Map[(int)n.Y, (int)n.X] == 0 || w.Map[(int)n.Y, (int)n.X] == -1))
                                {
                                    Fail = true;
                                    goto Finish1;
                                }
                            }
                        Finish1: if (Fail) continue;
                            Set(new Vector2(x, y), 2);
                            for (int a = 0; a < r.Floor.Count(); a++)
                            {
                                Set(new Vector2(x, y) + r.Floor[a], 2);
                            }
                            for (int a = 0; a < r.Wall.Count(); a++)
                            {
                                Set(new Vector2(x, y) + r.Wall[a], -1);
                            }
                ...
                for (int xi = 0; xi < w.MapSize.X; xi++)
                {
                    for (int yi = 0; yi < w.MapSize.Y; yi++)
                    {
                        ...
                        if (x < w.MapSize.X - 3)
                        {
                            ...
                            else if (w.Map[y, x + 2] != 2 && w.Map[y, x + 3] == 2)
                            {
                    ...
                    End:
                        if (y < w.MapSize.Y - 3)
                        {
                            ...
                            else if (w.Map[y + 2, x] != 2 && w.Map[y + 3, x] == 2)
                            {
            ...
            for (int x = 0; x < w.MapSize.X; x++)
            {
                ...
                {
                    if (w.Map[y, x] == 3 || w.Map[y, x] == 4) continue;
                    op = (double)(x + 1) / w.MapSize.X;
                    if (w.Map[y, x] == 2)
                    {
                        int r = rnd.Next(0, 1000);
                        if (r > 900) w.SpikeList.Add(new Spike(new Vector2(x, y)));
                        if (r > 780 && r <= 800) w.CrossBowList.Add(new CrossBow(new Vector2(x, y)) { Direction = rnd.Next(0, 4) });
                        if (r > 895 && r <= 900) w.EnemyList.Add(new Enemy(new Vector2(x, y)));
                        if (r > 600 && r <= 650) w.BlockList.Add(new Block(new Vector2(x, y), 0));
                        if (r > 0 && r <= 10) w.Mod[y, x] = 1;
                        if (r > 10 && r <= 20) w.Mod[y, x] = 2;
                        if (r > 20 && r <= 30) w.Mod[y, x] = 3;
                        if (r > 30 && r <= 40) w.Mod[y, x] = 4;
                        if (r > 40 && r <= 50) w.Mod[y, x] = 5;
                        if (r > 50 && r <= 60) w.Mod[y, x] = 6;
                        if (r > 60 && r <= 70) w.Mod[y, x] = 7;
                        if (r > 70 && r <= 80) w.Mod[y, x] = 8;
                        if (r > 80 && r <= 90) w.Mod[y, x] = 9;
                        if (r > 90 && r <= 100) w.Mod[y, x] = 10;
                        if (r > 100 && r <= 110) w.Mod[y, x] = 11;
                        if (r > 110 && r <= 120) w.Mod[y, x] = 12;
                        if (r > 890 && r <= 895) w.Mod[y, x] = 13;
                        continue;
                    }
                    for (int i = 0; i < 8; i++)
                    {
            ...
            Thread.Sleep(1000);
            GameState = 1;
            Play(Sources["BackMusic"], Buffers["BackMusic"]);
        }
        ...
        static Room LoadRoom(Bitmap b, int r)
        {
            ...
            switch (r)
            {
                case 1: b.RotateFlip(RotateFlipType.RotateNoneFlipX); break;
                case 2: b.RotateFlip(RotateFlipType.RotateNoneFlipY); break;
                case 3: b.RotateFlip(RotateFlipType.RotateNoneFlipX); b.RotateFlip(RotateFlipType.RotateNoneFlipY); break;
                case 4: b.RotateFlip(RotateFlipType.Rotate90FlipNone); break;
                case 5: b.RotateFlip(RotateFlipType.Rotate270FlipNone); break;
            }
            for (int n = 0; n < b.Width * b.Height; n++)
            {
                ...
                if (c == Color.FromArgb(255, 255, 0, 0)) room.Entrance = new Vector2(x, y);
                if (c == Color.FromArgb(255, 255, 255, 255)) room.Floor.Add(new Vector2(x, y));
                if (c == Color.FromArgb(255, 0, 255, 0)) room.Wall.Add(new Vector2(x, y));
            }
            if (room.Entrance.Y == 0) room.Direction = 0;
            if (room.Entrance.X == b.Width - 1) room.Direction = 1;
            if (room.Entrance.Y == b.Height - 1) room.Direction = 2;
            if (room.Entrance.X == 0) room.Direction = 3;
            room.Localise();
            return room;
        }
    }
